...
async function validateResetToken({ token }) {
    const account = await db_1.default.Account.findOne({
        where: { resetToken: token, resetTokenExpires: { [sequelize_1.Op.gt]: Date.now() } }
    });
    if (!account)
        throw 'Invalid token';
    return account;
...
async function update(id, params) {
    const account = await getAccount(id);
    if (params.email && params.email !== account.email && await db_1.default.Account.findOne({ where: { email: params.email } })) {
